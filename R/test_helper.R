...
build_dir_structure <- function(root = getwd()) {
  ...
  samples <- fragmentomics::parse_samplesheet(samplesheet, root)
  ...
  samples |> purrr::pmap(function(caseid, sampleid, timepoint, encoded_timepoint, datapath) {
    mpath <- file.path(datapath, "fragmentomics", "processed", "matrix", "source_1","CTCF")
    dir.create(mpath, showWarnings = FALSE, recursive = TRUE)
    file.copy(ctcf_example_file, mpath, overwrite = TRUE )
    mpath <- file.path(datapath, "fragmentomics", "processed", "matrix", "source_1","ELK4")
    dir.create(mpath, showWarnings = FALSE, recursive = TRUE)
