...
#'
#' This function generates a ggplot plot for the z-scores of housekeeping genes compared to random datasets.
...
#' @param bins An optional integer specifying the number of bins for the histogram. If NULL, breaks are calculated using Sturge's method.#' @param bins An optional integer specifying the number of bins for the histogram.
...
housekeeping_zscore_plot <- function(housekeeping, random, bins = NULL) {
  ...
  plot_subtitle <- paste("zscore=",
                         ...
                         sep = "")
  if (is.null(bins)) {
    breaks <- pretty(range(random$peak_length), n = nclass.Sturges(random$peak_length), min.n = 1)
  ...
  g1 <- g +
    ...
    ggrepel::geom_label_repel(data = housekeeping,
                              ggplot2::aes(x = peak_length, y = max(b$data[[1]]$count)),
                              color = "red",
